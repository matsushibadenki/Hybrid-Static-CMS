...
export async function writeSetupEnvironment(input: {
  ...
  const updates: Record<string, string> = {
    ...
    GOOGLE_MAPS_EMBED_API_KEY: envValue(config.googleMapsEmbedApiKey ?? ""),
    OPENSTREETMAP_TILE_URL: envValue(config.openStreetMapTileUrl),
    OPENSTREETMAP_ROUTING_URL: envValue(config.openStreetMapRoutingUrl),
