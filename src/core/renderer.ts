...
function card(post: PostRecord) {
  return `
    <article class="bunpress-card">
      <div class="bunpress-card__meta">
...
<html lang="en">
  <head>
    ...
    <style>
      ...
      .bunpress-list { display: grid; gap: 16px; }
      .bunpress-card { background: rgba(255,255,255,0.88); border: 1px solid rgba(0,0,0,0.08); border-radius: 24px; padding: 20px; }
      .bunpress-card__meta { display: flex; gap: 12px; color: #52606d; font-size: 0.85rem; margin-bottom: 10px; }
...
function renderList(title: string, posts: PostRecord[], pagination?: { page: number; totalPages: number }) {
  ...
  return pageTemplate(
    title,
    `<h1>${escapeHtml(title)}</h1><section class="bunpress-list">${cards || "<p>No posts published yet.</p>"}</section>${pager}`,
  );
}

function renderFragment(posts: PostRecord[]) {
  if (posts.length === 0) {
    return `<div class="bunpress-fragment"><p>No posts published yet.</p></div>`;
  }

  return `<div class="bunpress-fragment">${posts.map(card).join("")}</div>`;
}
...
function renderPageIndex(pages: PageRecord[]) {
  ...
  return pageTemplate(
    "CMS Managed Pages",
    `<h1>CMS Managed Pages</h1><section class="bunpress-list">${cards || "<p>No pages published yet.</p>"}</section>`,
  );
}
...
function renderEmbedScript() {
  return `(() => {
  const nodes = document.querySelectorAll("[data-bunpress-posts]");
  ...
  const mount = async (node) => {
    ...
    node.innerHTML = data.items.map((post) => {
      ...
      return '<article class="bunpress-embed-card">' +
